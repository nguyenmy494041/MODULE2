...
using System.IO;
using System.Text;
using static BaitapcuoiModule.Orders;
    ...
    class ListCustomer
    {
       ...
       static void Main(string[] args)
        ...
        static void AddOrder()
        {
            ...
            {
                id = customer.listorder[0].oredrId;
            ...
            do
            {
                ...
                if (orders.orders.Count == 0) { orders.orders.Add(order); }
                else
                {
                    int check = 0;
                    foreach (var item in orders.orders)
                    {
                        if (item.codeproduct == order.codeproduct)
                        {
        ...
        static void UpdateOrder()
        {
            try
            {
                ...
                if (pos != -1 && customer.listorder[pos].status == "Order")
                {
                    int choice = -2;
                    while (choice != 0)
                    {
                ...
                else Console.WriteLine("Not Found or Payed!");
        ...
        {
            var outfilePath = $@"E:\CODEGYM\Module2\BaitapcuoiModule\BaitapcuoiModule\Pay";
            ...
            if (pos != -1)
            {
                ...
                if (customer.listorder[pos].status == "Order")
                {
                    ...
                    string fileName = $@"Pay__{customer.listorder[pos].nameCustomer}__{DateTime.Now.ToString("yyyyMMddhhmm")}.json";
                    using (StreamWriter sw = File.CreateText(@$"{outfilePath}\{fileName}"))
                    {
                        var data = JsonConvert.SerializeObject(customer.listorder[pos]);
                        sw.Write(data);
                    }
                }
        ...
        static void WriteCreateDATA()
        {
            using(StreamWriter sw = File.CreateText($@"E:\CODEGYM\Module2\BaitapcuoiModule\BaitapcuoiModule\DATA.inputDATA"))
            {
                var data = JsonConvert.SerializeObject(customer);
                sw.Write(data);
            }
        }
        static Customer ReadDATA()
        {
            
            using(StreamReader sr = File.OpenText($@"E:\CODEGYM\Module2\BaitapcuoiModule\BaitapcuoiModule\DATA.inputDATA"))
            {
        ...
        {
            using (StreamWriter sw = File.AppendText($@"E:\CODEGYM\Module2\BaitapcuoiModule\BaitapcuoiModule\DATA.inputDATA"))
            {
                var data = JsonConvert.SerializeObject(customer);
                sw.Write(data);
            }
